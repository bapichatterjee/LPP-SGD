...
def prepare_dir(args):
    ...
    args.hpstring = (
        args.training_type.title()
        + args.scheduler_type.title()
        + args.model.title()
        + args.dataset.title()
        + "Bs"
        + str(args.train_bs)
        + "ProcBs"
        + str(args.train_processing_bs)
        + "Lr"
        + str(args.lr - np.fix(args.lr))[2:]
        + "AvFreq"
        + str(args.averaging_freq)
        + "NumNodes"
        + str(args.numnodes)
    )
    ...
    args.results_dir = (
        curr_dir + args.results_dir + args.hpstring + "_" + datetimestring
    )
    if args.clean_start:
...
def prepare_run_files(args):
    ...
    runtbfile.write(
        "tensorboard --bind_all --logdir=" + args.results_dir + " --port=6006 &\n"
    )
    runtbfile.close()
    ...
    flaskstartfile.write(
        "cp "
        + args.results_dir
        + "/result.html "
        + args.results_dir
        + "/rundir/templates/index.html\n"
    )
...
def add_dist_args(parser):
    ...
    dist_parser.add_argument(
        "--numnodes", type=int, default=2, help="number of nodes running dist"
    )
    dist_parser.add_argument(
        "--masters_share",
        type=int,
...
def add_lr_args(parser):
    ...
    lr_parser.add_argument(
        "--gamma", default=0.1, type=float, help="learning rate decay"
    )
    lr_parser.add_argument("--eta", default=0.001, type=float, help="LARS eta")
    lr_parser.add_argument(
        "--multiplier", default=4, type=int, help="warm up multiplier"
    )
    lr_parser.add_argument(
        "--lrdecaystep", default=30, type=int, help="learning rate decaystep"
    )
    lr_parser.add_argument(
        "--lrmilestone",
        nargs="+",
    ...
    lr_parser.add_argument(
        "--warm_up_epochs", default=0, type=int, help="learning rate decaystep"
    )
    lr_parser.add_argument(
        "--prepassmepochs", default=10, type=int, help="learning rate decaystep"
    )
    lr_parser.add_argument(
        "--baseline_lr", default=0.1, type=float, help="baseline learning rate"
    )


def add_averaging_args(parser):
    averaging_parser = parser.add_argument_group(title="averaging args")
    averaging_parser.add_argument(
        "--averaging_freq", type=int, default=4, help="averaging interval in seconds"
    )
...
def add_common_args(parser):
    ...
    common_parser.add_argument(
        "--gpus", nargs="+", default=[1], type=int, help="GPU ranks"
    )
    common_parser.add_argument("--lr", default=0.1, type=float, help="learning rate")
    common_parser.add_argument("--momentum", default=0.9, type=float, help="momentum")
    common_parser.add_argument(
        "--nesterov", default=False, action="store_true", help="nesterov momentum"
    )
    common_parser.add_argument(
        "--dampening", default=0, type=float, help="momentum dampening"
    )
    common_parser.add_argument(
        "--weight-decay", default=5e-4, type=float, help="L2 weight decay"
    )
    common_parser.add_argument(
        "--beta", default=0.5, type=float, help="randBeta probab"
    )
    common_parser.add_argument(
        "--model",
        default="res20",
    ...
    common_parser.add_argument(
        "--num-classes", type=int, default=1000, metavar="NC", help="Number of classes"
    )
    common_parser.add_argument(
        "--training-type",
        default="ddp",
    ...
    common_parser.add_argument(
        "--lars", action="store_true", default=False, help="enables LARS training"
    )
    common_parser.add_argument(
        "--distributed",
        action="store_true",
    ...
    common_parser.add_argument(
        "--pm", action="store_true", default=False, help="enables memory-pinning"
    )
    common_parser.add_argument(
        "--no-progressbar",
        action="store_true",
    ...
    common_parser.add_argument(
        "--clean-start", action="store_true", default=False, help="do test"
    )
    common_parser.add_argument(
