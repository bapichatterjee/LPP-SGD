...
class Bottleneck(nn.Module):
    def __init__(
        self, inplanes, planes, kernel_size, stride, expand, se_ratio, prob=1.0
    ):
        ...
        if expand == 1:
            ...
            self.conv1 = nn.Conv2d(
                inplanes, inplanes * expand, kernel_size=1, bias=False
            )
...
class MBConv(nn.Module):
    def __init__(
        ...
        layer.append(
            Bottleneck(inplanes, planes, kernel_size, stride, expand, se_ratio)
        )

        for l in range(1, repeat):
            if count_layer is None:
                layer.append(
                    Bottleneck(planes, planes, kernel_size, 1, expand, se_ratio)
                )
            else:
                # stochastic depth
