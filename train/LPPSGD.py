...
def get_indices(param_list, num_processes, mymaster_rank):
    ...
    for ind, r in enumerate(re):
        if ind > 0 and r >= chunkstart and re[ind - 1] < chunkstart:
            rankstart = ind + 1
        if ind < model_len - 2 and r <= chunkstop + 1 and re[ind + 1] > chunkstop:
            rankstop = ind + 2
...
def average(net, args, minibatch_counter):
    ...
    while True:
        ...
        maxepoch = torch.tensor([minibatches, -last_averaged_at]).float().cuda()
        ...
        if maxminibatches >= args.trainloaderlength * args.epochs:
            print("Reached MaxEpoch at rank ", args.commrank, maxepoch, minibatches)
            break
        avg_freq = (
            1
            if maxminibatches * 1.0 / args.trainloaderlength < args.pre_post_epochs
            else args.averaging_freq
        )
...
class opt(object):
    ...
    def step(self, grad, is_passm):
        ...
        for index, p in enumerate(self.parameters):
            if is_passm == 1 and (index < self.rankstart or index >= self.rankstop):
                continue
            d_p = grad[counter]
            counter += 1
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        for i in range(0, len(inputs), args.train_processing_bs):
            ...
            if len(inputs) != args.train_processing_bs:
                loss.div_(math.ceil(float(len(inputs)) / args.train_processing_bs))
            """Gradient Computation depending on if it is Partial"""
            if is_passm[0] == 1:
                grad = torch.autograd.grad(
                    loss, optimizer.parameters[rankstart:rankstop]
                )
                assert len(grad) == (rankstop - rankstart), "Length of grad issue!"
            else:
        ...
        with minibatch_counter.get_lock():
            ...
            with last_tested_at.get_lock():
                if (
                    minibatches - last_tested_at.value
                    >= args.test_freq * args.trainloaderlength
                ):
                    do_test = True
...
def run(args):
    ...
    for m in range(args.num_processes):
        rank_tuple.append(get_indices(param_list, args.num_processes, m))
    print(rank_tuple)
